...
dat <- read_csv("contraceptive_use.csv")
 ...
 dat2<-dat%>%
   mutate(observation = (start_date+end_date)/2)%>%
 ...
 plot_cp <- function(dat2,est,iso_code,CI= 95) {
   ...
   p_time <- ggplot() +
     ...
     geom_point(data = dat_time, aes(x = observation, y = contraceptive_use_modern), color = "red")+ 
     ...
     p <- p_time +  
       geom_ribbon(data = est_time, stat = "identity", aes(x = Year, ymin = L80, ymax = U80, alpha = 0.3))
   }
    else {
      p <- p_time +
        geom_ribbon(data = est_time, stat = "identity", aes(x = Year, ymin = L95, ymax = U95, alpha = 0.3))
    }
     return(p_time)
}
```
