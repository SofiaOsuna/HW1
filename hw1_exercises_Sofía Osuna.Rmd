...
plot_cp <- function(dat2,est,iso_code,CI= 95){
  ...
  dat2<-dat%>%
   ...
   p_time <- ggplot() + 
     geom_point(data = dat_time, aes(x = ref_time, y = contraceptive_use_modern)) +
     labs(x = "Time", y = "Modern use (%)", title = paste("mCPR in", est_time$`Country or area`[1]))
   
if(is.na(CI)==T) {
    p <- p_time + geom_line(data = est_time, aes(x = Year, y = Median))
}
  else if(CI == 80){
    p <- p_time + geom_ribbon(data = est_time, stat = "identity", aes(x = Year, y = Median, ymin = L80, ymax = U80), alpha = 0.3) +
      geom_line(data = est_time, aes(x = Year, y = Median), color = "blue")
      
  }
   else{
     p <- p_time + geom_smooth(data = est_time, stat = "identity", aes(x = Year, y = Median, ymin = L95, ymax = U95), alpha = 0.3) +
       geom_line(data = est_time, aes(x = Year, y = Median), color = "blue")
   }
   return(p)
 }
...
https://github.com/SofiaOsuna/HW1/blob/main/hw1_starter.Rmd



Collaborators: Omar Jouini, Sylvain Casimir and Sarah Mirza
